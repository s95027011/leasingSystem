...
class OrderViewSet(mixins.CreateModelMixin,
    ...
    @action(detail=False, methods=['post'])
    def list_order_view(self, request):
        all_num = 0
        order_id = request.data['id']
        order_item = Order.objects.filter(id=order_id)
        query_order_item = order_item.values('rent_datetime', 'order_datetime')
        order_data = {}
        query_product_item = Order.objects.filter(id=order_id).values('item__product__product_name',
                                                                      'item__product__product_size',
                                                                      'item__product__product_price',
                                                                      'item__product__product_image')
        product_item = query_product_item.annotate(product_name=F('item__product__product_name'),
                                                   product_size=F('item__product__product_size'),
                                                   product_price=F('item__product__product_price'),
                                                   product_image=F('item__product__product_image'))
        for num in range(len(product_item)):
            product_item_data = product_item[num]
            old_key = ['item__product__product_name', 'item__product__product_size', 'item__product__product_price',
                       'item__product__product_image']
            for key in old_key:
            ...
            all_num += 1
            order_data.update({'all_num': all_num})
